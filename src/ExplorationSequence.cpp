#include <fixed-base-exploration/ExplorationSequence.h>
...
ExplorationSequence::ExplorationSequence(std::shared_ptr<ocra::Controller> _ctrl, std::shared_ptr<ocra::Model> _model)
: ctrl(_ctrl)
, model(_model)
{
...
ExplorationSequence::~ExplorationSequence()
{

}

void ExplorationSequence::close()
{
    l_hand_port.close();
...
void ExplorationSequence::initialize()
{
    ...
    */
    ...
    Eigen::VectorXd weights_leftHand = Eigen::VectorXd::Ones(3);

    //  rightHand
    double Kp_rightHand = 10.0;
    double Kd_rightHand = 2.0 *sqrt(Kp_rightHand);
    Eigen::VectorXd weights_rightHand = Eigen::VectorXd::Ones(3);
    ...
    */
    ...
    // ocra_icub::getHomePosture(*model, nominal_q);

    // fullPostureTask = std::make_shared<ocra::FullPostureTaskManager>(*ctrl, *model, "fullPosture", ocra::FullState::INTERNAL, Kp_fullPosture, Kd_fullPosture, weight_fullPosture, nominal_q, usesYARP);
    fullPostureTask = std::make_shared<ocra::FullPostureTaskManager>(*ctrl, *model, "fullPosture", ocra::FullState::INTERNAL, Kp_fullPosture, Kd_fullPosture, weight_fullPosture, usesYARP);
    ...
    partialPostureTask = std::make_shared<ocra::PartialPostureTaskManager>(*ctrl, *model, "torsoPosture", ocra::FullState::INTERNAL, torso_indices, Kp_torsoPosture, Kd_torsoPosture, weight_torsoPosture, torsoTaskPosDes, usesYARP);


    //  leftHand
    Eigen::Vector3d l_handDisp(0.05, 0.0, 0.0); // Moves the task frame to the center of the hand.
    leftHandTask = std::make_shared<ocra::SegCartesianTaskManager>(*ctrl, *model, "leftHand", "l_hand", l_handDisp, ocra::ECartesianDof::XYZ, Kp_leftHand, Kd_leftHand, weights_leftHand, usesYARP);

    //  rightHand
    Eigen::Vector3d r_handDisp(0.05, 0.0, 0.0); // Moves the task frame to the center of the hand.
    rightHandTask = std::make_shared<ocra::SegCartesianTaskManager>(*ctrl, *model, "rightHand", "r_hand", r_handDisp, ocra::ECartesianDof::XYZ, Kp_rightHand, Kd_rightHand, weights_rightHand, usesYARP);
    ...
    */
    ...
    leftHandTrajectory = std::make_shared<ocra::GaussianProcessTrajectory>();
    rightHandTrajectory = std::make_shared<ocra::GaussianProcessTrajectory>();
...
void ExplorationSequence::update()
{
    ...
    if (initTrigger) {
        resetTimeLeft = time;
        resetTimeRight = time;
        generateNewWaypoints(lHandIndex);
        initTrigger = false;
        generateNewWaypoints(rHandIndex);
    ...
    if ( (std::abs(time - resetTimeLeft) >= TIME_LIMIT) || (attainedGoal(lHandIndex)) )
    {
        generateNewWaypoints(lHandIndex);
        resetTimeLeft = time;
    }
    else
    {
        ...
        leftHandTask->setWeight(desiredWeights_leftHand);

    }

    if ( (std::abs(time - resetTimeRight) >= TIME_LIMIT) || (attainedGoal(rHandIndex)) )
    {
        generateNewWaypoints(rHandIndex);
        resetTimeRight = time;
    ...
    // std::cout << "Current position: " << model->getSegmentPosition(lHandIndex).getTranslation().transpose()<< std::endl;
...
bool ExplorationSequence::attainedGoal(int segmentIndex)
{
    double error;
    ...
    if (segmentIndex==lHandIndex) {
        ...
        std::cout << "[ERROR] ExplorationSequence::attainedGoal - segment name doesn't match either l_hand or r_hand" << std::endl;
        return false;
    }
...
Eigen::VectorXd ExplorationSequence::mapVarianceToWeights(Eigen::VectorXd& variance)
{
    double beta = 1.0;
...
void ExplorationSequence::generateNewWaypoints(int segmentIndex)
{
    std::cout << "\n==================" << std::endl;
    Eigen::VectorXd startPoint = model->getSegmentPosition(segmentIndex).getTranslation();
    if (segmentIndex==lHandIndex) {
...
Eigen::VectorXd ExplorationSequence::generateTarget(int segmentIndex)
{
